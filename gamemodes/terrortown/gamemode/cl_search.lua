...
local function IconForInfoType(t, data)
	...
	if symbols then
		TypeToMat = {
			...
			eq_radar = "radar",
			eq_disg = "disguise",
			role = { [ROLE_TRAITOR] = "sym_tra", [ROLE_DETECTIVE] = "sym_det", [ROLE_MERCENARY] = "sym_mer", [ROLE_HYPNOTIST] = "sym_hyp", [ROLE_GLITCH] = "sym_gli", [ROLE_JESTER] = "sym_jes", [ROLE_PHANTOM] = "sym_pha", [ROLE_WRAITH] = "sym_zom", [ROLE_VAMPIRE] = "sym_vam", [ROLE_SWAPPER] = "sym_swa", [ROLE_ASSASSIN] = "sym_ass", [ROLE_KILLER] = "sym_kil", [ROLE_INNOCENT] = "sym_inn" },
			c4 = "code",
			dmg = DmgToMat,
			wep = WeaponToIcon,
			head = "head",
		...
		TypeToMat = {
			...
			eq_radar = "radar",
			eq_disg = "disguise",
			role = { [ROLE_TRAITOR] = "let_tra", [ROLE_DETECTIVE] = "let_det", [ROLE_MERCENARY] = "let_mer", [ROLE_HYPNOTIST] = "let_hyp", [ROLE_GLITCH] = "let_gli", [ROLE_JESTER] = "let_jes", [ROLE_PHANTOM] = "let_pha", [ROLE_WRAITH] = "let_zom", [ROLE_VAMPIRE] = "let_vam", [ROLE_SWAPPER] = "let_swa", [ROLE_ASSASSIN] = "let_ass", [ROLE_KILLER] = "let_kil", [ROLE_INNOCENT] = "let_inn" },
			c4 = "code",
			dmg = DmgToMat,
			wep = WeaponToIcon,
			head = "head",
...
function PreprocSearch(raw)
	...
	for t, d in pairs(raw) do
		...
		elseif t == "role" then
			...
			elseif d == ROLE_MERCENARY then
				search[t].text = T("search_role_m")
			elseif d == ROLE_HYPNOTIST then
				search[t].text = T("search_role_h")
			elseif d == ROLE_GLITCH then
				search[t].text = T("search_role_g")
			elseif d == ROLE_JESTER then
				search[t].text = T("search_role_j")
			elseif d == ROLE_PHANTOM then
				search[t].text = T("search_role_p")
			elseif d == ROLE_WRAITH then
				search[t].text = T("search_role_z")
			elseif d == ROLE_VAMPIRE then
				search[t].text = T("search_role_v")
			elseif d == ROLE_SWAPPER then
				search[t].text = T("search_role_s")
			elseif d == ROLE_ASSASSIN then
